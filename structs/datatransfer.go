...
func (dd DataTransfer) GetValueOfTableRow(rows *sql.Rows) reflect.Value {
	...
	for _, idx := range dd.dataIdx {
		...
		if isStruct {
			...
			// 2022年3月30日 以下逻辑已经想不清楚为什么这么写了，优化去掉
...
func GetDataTransfer(key string, columns []string, model StructModel) DataTransfer {
	//TODO 未考虑简单对象传入的情况，将结果集的列和model的列做拟合的时候,必然会存在表列和columns不一致的情况.这个时候需要我们创造一个DataTransfer,Data,并且将datatransfer缓存到静态map中,后续直接从map中取用,无需再次优化
