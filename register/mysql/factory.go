...
func init() {
	register.Register("mysql", &factory)
	InitMysqlFactory()
}
func InitMysqlFactory() {
	...
	funcMap[defines.Query] = func(models ...defines.TableModel) []defines.SqlProto {
		...
		result = append(result, defines.SqlProto{PreparedSql: sql, Data: datas})
		return result
	}
	funcMap[defines.Update] = func(models ...defines.TableModel) []defines.SqlProto {
		...
		for _, model := range models {
			...
			result = append(result, defines.SqlProto{sql, datas})
		}
...
func (m Factory) GetColumns(tableName string, db *sql.DB) ([]defines.Column, error) {
	...
	for rows.Next() {
		...
		if er == nil {
			columns = append(columns, defines.Column{ColumnName: columnName, ColumnType: columnType, Primary: columnKey == "PRI", PrimaryAuto: columnKey == "PRI" && extra == "auto_increment"})
		} else {
			return nil, er
		}
	}
